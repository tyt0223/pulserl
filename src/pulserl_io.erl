...
-export([decode_command/1, decode_metadata/1, encode/3, encode_command/1]).
...
encode(InnerCommand, #'MessageMetadata'{} = Metadata, Payload) when is_binary(Payload) ->
    ...
    TotalSize =
        ?CMD_SIZE_LEN
        + CommandSize
        + ?MAGIC_NUMBER_SIZE_LEN
        + ?CHECKSUM_SIZE_LEN
        + ?METADATA_SIZE_LEN
        + MetadataSize
        + byte_size(Payload),
    iolist_to_binary([write_int32(TotalSize),
                      write_int32(CommandSize),
                      EncodedCommand,
                      write_int16(?MAGIC_NUMBER),
                      write_int32(crc32c(MetadataSize_Metadata_Payload)),
                      MetadataSize_Metadata_Payload]).
...
decode_command(Data) when is_binary(Data) ->
    ...
    ['BaseCommand', _Type | WrappedCommands] =
        lists:filter(fun(Val) -> Val /= undefined end, tuple_to_list(BaseCommand)),
...
verify_checksum(HeadersAndPayload) ->
    case has_checksum(HeadersAndPayload) of
        true ->
            <<_MagicNumber:2/binary, Checksum:32/unsigned-integer, Rest/binary>> =
                HeadersAndPayload,
...
wrap_to_base_command(InnerCmd) when is_tuple(InnerCmd) ->
    ...
    BaseCommand = #'BaseCommand'{type = Type},
