    ...
    Metadata =
        #'MessageMetadata'{event_time = EventTime,
                           ...
                           num_messages_in_batch =
                               NumMessages}, %% Must be `undefined` for non-batch messages
    {SendCmd, Metadata}.
...
new_redeliver_unack_messages(ConsumerId, MessageIds) when is_list(MessageIds) ->
    #'CommandRedeliverUnacknowledgedMessages'{consumer_id = ConsumerId,
                                              message_ids =
                                                  [#'MessageIdData'{ledgerId = LedgerId,
                                                                    entryId = EntryId}
                                                   || #messageId{ledger_id = LedgerId,
                                                                 entry_id = EntryId}
                                                          <- MessageIds]}.

new_connect() ->
    #'CommandConnect'{protocol_version = 10,
...
new_lookup_topic(Topic, Authoritative) ->
    #'CommandLookupTopic'{topic = Topic, authoritative = Authoritative}.

get_request_id(Command) when is_tuple(Command) ->
    case Command of
        #'CommandSuccess'{} ->
            Command#'CommandSuccess'.request_id;
        #'CommandProducerSuccess'{} ->
            Command#'CommandProducerSuccess'.request_id;
        #'CommandLookupTopicResponse'{} ->
            Command#'CommandLookupTopicResponse'.request_id;
...
set_request_id(Cmd, ReqId) when is_tuple(Cmd) ->
    case Cmd of
        #'CommandSeek'{} ->
            Cmd#'CommandSeek'{request_id = ReqId};
        #'CommandProducer'{} ->
            Cmd#'CommandProducer'{request_id = ReqId};
        #'CommandGetSchema'{} ->
            Cmd#'CommandGetSchema'{request_id = ReqId};
        #'CommandSubscribe'{} ->
            Cmd#'CommandSubscribe'{request_id = ReqId};
        #'CommandLookupTopic'{} ->
            Cmd#'CommandLookupTopic'{request_id = ReqId};
        #'CommandUnsubscribe'{} ->
            Cmd#'CommandUnsubscribe'{request_id = ReqId};
        #'CommandCloseProducer'{} ->
            Cmd#'CommandCloseProducer'{request_id = ReqId};
        #'CommandCloseConsumer'{} ->
            Cmd#'CommandCloseConsumer'{request_id = ReqId};
        #'CommandGetLastMessageId'{} ->
            Cmd#'CommandGetLastMessageId'{request_id = ReqId};
        #'CommandGetTopicsOfNamespace'{} ->
            Cmd#'CommandGetTopicsOfNamespace'{request_id = ReqId};
