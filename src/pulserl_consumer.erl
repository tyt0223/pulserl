...
seek(Pid, {LedgerId, EntryId} = MsgId)
    when is_pid(Pid) andalso is_integer(LedgerId) andalso is_integer(EntryId) ->
...
-record(state,
        {state, connection :: pid(), consumer_id :: integer(),
         ...
         consumer_priority_level :: atom(), consumer_subscription_type :: atom(),
         ...
         re_init_timer, flow_permits = 0, queue_size = 1000, queue_refill_threshold = 1000,
         incoming_messages = queue:new() :: queue:queue(),
         %% Dead Letter Policy
         dead_letter_topic_producer :: pid(),
         dead_letter_topic_messages = ?UNDEF :: #{},
         dead_letter_topic_max_redeliver_count :: integer(), dead_letter_topic_name :: string(),
         retry_letter_topic_name :: string(), children_poll_sequence = 0,
         %%% For acknowledgements
         batch_ack_trackers = #{} :: #{},
         un_ack_message_ids = #{} ::
             #{}, %% messageId => ack_timeout_time
         acknowledgment_timeout_timer :: reference(),
         acknowledgment_timeout ::
             non_neg_integer(), %% [1000, ...), 0 to disable
         nack_message_redelivery_delay,  %% [100, ...)
         nack_messages_to_redeliver = [], nack_message_redelivery_timer :: reference(),
         ...
         acknowledgments_send_timer_tick ::
             non_neg_integer(), %% [0, ...) 0 to disable delayed ack (ack immediately)
         max_pending_acknowledgments = 1000 ::
             non_neg_integer(), %% [0, ...) 0 to disable delayed ack (ack immediately)
...
handle_call({acknowledge, _, true},
            _From,
            #state{consumer_subscription_type = ?SHARED_SUBSCRIPTION} = State) ->
    {reply, ?ERROR_ACK_TYPE_NOT_ALLOWED, State};
%% The parent
handle_call({acknowledge, #messageId{topic = Topic} = MsgId, _Cumulative},
            _From,
            #state{parent_consumer = ?UNDEF,
                   partition_count = Pc,
                   topic = ParentTopic} =
                State)
    when Pc > 0 ->
...
%% The child/no-child-consumer
handle_call({negative_acknowledge, #messageId{topic = TopicStr} = MsgId},
            _From,
            #state{topic = Topic} = State) ->
    case TopicStr == topic_utils:to_string(Topic) of
        false ->
...
handle_call(redeliver_unack_messages, _From, #state{parent_consumer = Parent} = State) ->
    {Reply, State3} =
        if not is_pid(Parent) ->
               foreach_child(fun(Pid) -> redeliver_unack_messages(Pid) end, State),
               {ok, State};
           true ->
               ...
               case pulserl_conn:send_simple_command(State#state.connection, Command) of
                   ...
                   _ ->
                       ...
                       State2 =
                           State#state{batch_ack_trackers = #{},
                                       un_ack_message_ids = #{},
...
handle_cast({close, AttemptRestart}, #state{partition_count = PartitionCount} = State) ->
    if AttemptRestart ->
           State2 =
               if PartitionCount > 0 ->
                      error_logger:info_msg("Restarting ~p partitioned consumers "
                                            "of ~p",
                                            [PartitionCount, self()]),
                      close_children(State);
                  true ->
                      error_logger:info_msg("Restarting consumer(~p) with subscription "
                                            "[~p]",
        ...
        {Metadata, Payload} ->
            {MetadataAndMessages, State2} =
                case commands:has_messages_in_batch(Metadata) of
                    false ->
                        {[{Metadata,
                           pulserl_utils:new_message(Topic,
                                                     MessageId,
                                                     Metadata,
                                                     Payload,
                                                     RedeliveryCount)}],
                    ...
                    _ ->
                        ...
                        {SingleMetaAndMessages, _} =
                            lists:foldr(%% Fold from the right to make sure we don't change the list order.
                                        %% Also, the index assignment starts from the end, `BatchSize - 1`
                                        fun({SingleMetadata, ActualPayload},
                                            {MessageList, BatchIndex}) ->
                                           BatchMessageId =
                                               pulserl_utils:new_message_id(Topic,
                                                                            MsgId,
                                                                            BatchIndex,
                                                                            BatchSize),
                                           {[{SingleMetadata,
                        ...
                        BatchTracker =
                            sets:from_list(
                                lists:seq(0, LastBatchIndex)),
                        NewBatchAckTrackers =
                            maps:put(NewTrackerKey, BatchTracker, State#state.batch_ack_trackers),
                        {SingleMetaAndMessages,
                         State#state{batch_ack_trackers = NewBatchAckTrackers}}
                end,
...
cancel_all_timers(#state{} = State) ->
    State#state{re_init_timer = do_cancel_timer(State#state.re_init_timer),
                acknowledgment_timeout_timer =
                    do_cancel_timer(State#state.acknowledgment_timeout_timer),
                acknowledgments_send_timer =
                    do_cancel_timer(State#state.acknowledgments_send_timer),
                nack_message_redelivery_timer =
                    do_cancel_timer(State#state.nack_message_redelivery_timer)}.
...
start_nack_redelivery_timer(#state{nack_message_redelivery_timer_tick = Time} = State) ->
    State#state{nack_message_redelivery_timer =
                    self_send_after(?REDELIVER_NACK_MESSAGES, Time)}.
...
start_acknowledgments_send_timer(#state{acknowledgments_send_timer_tick = 0} = State) ->
    State;
start_acknowledgments_send_timer(#state{acknowledgments_send_timer_tick = Time} =
                                     State) ->
...
start_acknowledgment_timeout_timer(#state{acknowledgment_timeout = Time} = State) ->
    State#state{acknowledgment_timeout_timer =
                    self_send_after(?ACKNOWLEDGMENT_TIMEOUT, Time div 4)}.
...
redeliver_ack_timeout_messages(#state{topic = Topic, consumer_subscription_name = SubscriptionName}=State) ->
    ...
    {MessagesToRedeliver2, RestOfUnAckMessageIds} =
        maps:fold(fun(MsgId, TimeoutTime, {MessagesToRedeliver, UnAckMessageIds2}) ->
...
redeliver_nack_messages(State) ->
    NowMillis = erlwater_time:milliseconds(),
    {MessagesToRedeliver, RestOfNegAcknowledgments} =
        lists:partition(fun({WakeUpTime, _MsgId}) -> WakeUpTime =< NowMillis end,
                        State#state.nack_messages_to_redeliver),
...
handle_batch_acknowledgement(#messageId{batch = #batch{index = Index}} = MsgId,
                             Cumulative,
                             State) ->
    ...
    case maps:find(TrackerKey, State#state.batch_ack_trackers) of
        {ok, Tracker} ->
            ...
            case sets:is_empty(NewIndicesTracker) orelse Cumulative of
                ...
                false ->
                    ...
                    NewBatchTracker =
                        maps:put(TrackerKey, NewIndicesTracker, State2#state.batch_ack_trackers),
        ...
        _ ->
            ...
            error_logger:warning_msg("The batch that the message id: ~p belongs "
                                     "to has already been acknowledged",
                                     [MsgId]),
...
send_acknowledgment(MsgId,
                    _Cumulative,
                    #state{pending_acknowledgments = PendingAcknowledgments,
                           max_pending_acknowledgments = MaxPendingAcknowledgments} =
                        State) ->
...
do_send_ack_now(#messageId{} = MsgId,
                Cumulative,
...
do_send_pending_acknowledgements(#state{consumer_id = ConsumerId,
...
send_actual_command(#'CommandAck'{} = Command,
                    MessageIds,
                    #state{connection = Cnx} = State)
    when is_list(MessageIds) ->
    case pulserl_conn:send_simple_command(Cnx, Command) of
        {error, _} = Error ->
            lists:foldl(fun(MsgId, State0) -> on_message_ack_fail(Error, MsgId, State0) end,
                        State,
                        MessageIds);
        _ ->
            State
...
message_id_2_batch_ack_tracker_key(#messageId{ledger_id = LedgerId,
                                              entry_id = EntryId}) ->
    {LedgerId, EntryId}.
...
send_message_to_dead_letter_topic1(State) ->
    maps:fold(fun(_MsgId, Messages, State2) ->
...
do_send_to_dead_letter_topic1(Message, State) ->
    ...
    error_logger:warning_msg("Giving up processsing of message {legderId=~p"
                             ...
                             "from consumer {topic=~s, subscription=~s, "
                             "pid=~p}",
...
untrack_message(#messageId{} = MsgId,
                false,
                #state{un_ack_message_ids = UnAckMgs} = State) ->
...
untrack_message(#messageId{} = MsgId, _Cumulative, State) ->
    ...
    UnAcknowledgeMsgIds =
        maps:filter(fun(MsgId0, _) -> MsgId0 > MsgId end, State#state.un_ack_message_ids),
    State#state{un_ack_message_ids = UnAcknowledgeMsgIds}.

handle_message_error(MessageId, Error, State) ->
    error_logger:error_msg("Consumer message: ~p error: ~p", [MessageId, Error]),
    State.
...
child_consumer_exited(ExitedPid, Reason, State) ->
    case maps:get(ExitedPid, State#state.child_to_partition, false) of
        false ->
            ...
            {error, Reason};
        Partition ->
            error_logger:warning_msg("Consumer=[~s ~p] exited abnormally due "
                                     "to reason = '~p'. Restarting...",
                                     [topic_utils:partition_str(State#state.topic, Partition),
                                      self(),
                                      Reason]),
            State2 =
                State#state{partition_to_child =
                                maps:remove(Partition, State#state.partition_to_child),
                            child_to_partition =
                                maps:remove(ExitedPid, State#state.child_to_partition)},
...
reinitialize(#state{state = ?UNDEF} = State) ->
    case initialize(State) of
        {error, Reason} ->
            ...
            State#state{re_init_attempts = State#state.re_init_attempts + 1,
        ...
        LogicalAddress when is_list(LogicalAddress) ->
            case pulserl_client:get_broker_connection(LogicalAddress) of
                {ok, Pid} ->
                    ...
                    case subscribe_to_topic(State#state{connection = Pid, consumer_id = Id}) of
                        #state{} = State2 ->
...
subscribe_to_topic(State) ->
    ...
    Subscribe =
        #'CommandSubscribe'{consumer_id = State#state.consumer_id,
                            ...
                            initialPosition =
                                to_pulsar_initial_pos(State#state.consumer_initial_position),
                            priority_level =
                                case SubType of
                                    ?SHARED_SUBSCRIPTION ->
                                        State#state.consumer_priority_level;
                                    ?FAILOVER_SUBSCRIPTION ->
                                        State#state.consumer_priority_level;
                                    _ ->
                                        ?UNDEF
                                end},
    case pulserl_conn:send_simple_command(State#state.connection, Subscribe) of
        {error, _} = Err ->
            Err;
        #'CommandSuccess'{} ->
...
increase_flow_permits(State, Increment) ->
    ...
    if NewPermits >= State#state.queue_refill_threshold ->
           case send_flow_permits(State, NewPermits) of
               {error, _} = Error ->
                   error_logger:error_msg("Error: ~p on sending flow permits: [~p]) "
                                          "in consumer: [~p] as: ~s",
                                          [Error,
                                           NewPermits,
                                           self(),
...
send_flow_permits(#state{connection = Cnx} = State, NumberOfMessages) ->
    Permit =
        #'CommandFlow'{consumer_id = State#state.consumer_id, messagePermits = NumberOfMessages},
    ...
    case Err of
        {error, _} ->
            [pulserl_consumer:close(Pid, false)
             || {_, Pid} <- maps:keys(State2#state.child_to_partition)],
            Err;
        ?UNDEF ->
            State2
    end.
        ...
        {ok, Pid} ->
            {Pid,
             State#state{partition_to_child = maps:put(Index, Pid, State#state.partition_to_child),
                         child_to_partition =
                             maps:put(Pid, Index, State#state.child_to_partition)}};
        {error, _} = Error ->
            case Retries > 0 of
                true ->
                    create_child_consumer(Retries - 1, Index, State);
                _ ->
                    Error
            end
    end.
...
validate_options(Options) when is_list(Options) ->
    ...
    lists:foreach(fun ({parent_consumer, _} = Opt) ->
                          Opt;
                      ...
                      ({initial_position, V} = _Opt) ->
                          case V of
                              ...
                              Pos when is_integer(Pos) ->
                                  ok;
                              _ ->
                                  error({invalid_initial_position, V}, [Options])
                          end;
                      ({subscription_type, V} = _Opt) ->
                          case V of
                              ...
                              ?KEY_SHARED_SUBSCRIPTION ->
                                  ok;
                              _ ->
                                  error({invalid_subscription_type, V}, [Options])
                          end;
                      ({queue_size, _} = Opt) ->
...
partition_consumers_to_poll(undefined,
                            #state{partition_count = PartitionCount,
                                   children_poll_sequence = Sequence} =
                                State) ->
    HeadPartition = Sequence rem State#state.partition_count,
    TailPartitions =
        lists:foldl(fun (I, Acc) when I /= HeadPartition ->
                            [I | Acc];
                        (_, Acc) ->
                            Acc
                    end,
