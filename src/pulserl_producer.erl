...
send(Pid, #producerMessage{} = Message, Callback)
    ...
    {CallerFun, CallReturn} =
        if is_function(Callback) ->
               {fun() -> gen_server:call(Pid, {send_message, Callback, Message}, ?CALL_TIMEOUT) end,
                ok};
           true ->
               ClientRef = erlang:make_ref(),
               {fun() ->
                   gen_server:call(Pid, {send_message, {self(), ClientRef}, Message}, ?CALL_TIMEOUT)
                end,
                ClientRef}
        end,
    try
...
sync_send(Pid, #producerMessage{} = Message, Timeout)
    ...
    case gen_server:call(Pid, {send_message, {self(), ClientRef}, Message}, Timeout2 + 2000)
    of
        {redirect, ChildProducerPid} ->
...
-record(state,
        {state, connection :: pid(), producer_id :: integer(),
         ...
         topic :: #topic{},
         sequence_id :: integer(), partition_count = 0 :: non_neg_integer(),
         partition_to_child = dict:new(), child_to_partition = dict:new(),
         partition_routing_mode :: atom() | {atom(), atom()},
         partition_to_route_to :: non_neg_integer(), batching_requests = queue:new(),
         pending_requests = queue:new(), seqId2waiters = #{} :: #{}, batch_send_timer,
         re_init_timer,
         %%Config
         options :: list(),
         batch_enable :: boolean(), max_batched_messages :: integer(),
         batch_max_delay_ms :: integer(), max_pending_requests :: integer(),
...
init([#topic{} = Topic, Opts]) ->
    ...
    State =
        #state{topic = Topic,
               ...
               max_pending_requests_across_partitions =
                   proplists:get_value(max_pending_requests_across_partitions, Opts, 100000)},
    case initialize(State#state{sequence_id = State#state.producer_initial_sequence_id}) of
        {error, Reason} ->
            {stop, Reason};
        NewState ->
        ...
        ?UNDEF ->
            if State#state.send_timeout > 0 ->
                   error_logger:warning_msg("Received a command receipt for in producer: "
                                            ...
                                            [self(), SequenceId]);
               true ->
                   error_logger:info_msg("Received a command receipt for timed-out "
                                         "message; sequence-id = ~p",
                                         [SequenceId])
            end,
            State;
        Waiters ->
...
choose_partition_producer(Key, #state{partition_count = PartitionCount} = State) ->
    {Partition, State2} =
            ...
            {M, F} ->
                case catch M:F(Key, PartitionCount) of
                    Int when is_integer(Int) andalso Int >= 0 andalso Int < PartitionCount ->
                        {Int, State};
                    Other ->
                        error_logger:error_msg("The custom router mode: ~p produces "
                                               "invalid result: ~p. It must return an "
                                               "integer in the range: [0, ~p). Resorting "
                                               "to random routing",
    ...
    if State#state.batch_enable andalso Message#producerMessage.deliver_at_time == ?UNDEF ->
           case add_request_to_batch(Request, State) of
               {notfull, NewRequestQueue} ->
                   ...
                   {ok,
                    send_batch_if_possible(false,
                                           State#state{batching_requests = NewRequestQueue})};
               ...
               {fulled, NewRequestQueue} ->
                   ...
                   {ok,
                    send_batch_if_possible(false, State#state{batching_requests = NewRequestQueue})}
           end;
       true ->
...
send_batch_if_possible(ForceBatch,
                       #state{max_batched_messages = MaxBatchedMessages,
                              batching_requests = RequestQueue} =
                           State) ->
    ...
    case get_messages_to_batch(NumberOfMessagesToBatch, State) of
        {[], _} ->
            State; %% Not enough messages
        {NextBatch, NewRequestQueue} ->
            State2 = State#state{batching_requests = NewRequestQueue},
...
get_messages_to_batch(Size, #state{batching_requests = BatchingRequests} = State) ->
    case queue:len(BatchingRequests) >= Size of
        true ->
            ...
            update_pending_count_across_partitions(State, -queue:len(NextBatchQueue)),
            {queue:to_list(NextBatchQueue), RemainingQueue};
        _ ->
            {[], BatchingRequests}
    end.

resend_messages(#state{topic = Topic,
                       pending_requests = PendingReqs,
                       seqId2waiters = SeqId2Waiters} =
                    State) ->
    Len = queue:len(PendingReqs),
    if Len > 0 ->
           {SeqId2Waiters1, RequestsToResend1} =
               lists:foldl(fun({_, {SeqId0, Requests0}}, {SeqId2Waiters0, RequestsToResend0}) ->
                              {maps:remove(SeqId0, SeqId2Waiters0), Requests0 ++ RequestsToResend0}
                           end,
                           {SeqId2Waiters, []},
                           queue:to_list(PendingReqs)),
           error_logger:info_msg("Re-sending ~p messages from producer=~p, "
                                 "topic=~p",
                                 [length(RequestsToResend1), self(), topic_utils:to_string(Topic)]),
           send_batch_messages(RequestsToResend1,
                               State#state{seqId2waiters = SeqId2Waiters1,
...
send_batch_messages(RequestsToBatch,
                    #state{sequence_id = SeqId,
    ...
    {FinalSeqId, BatchPayload2} =
        lists:foldl(fun({_, Msg}, {SeqId0, BatchPayload0}) ->
                       ...
                       BatchPayload1 =
                           erlang:iolist_to_binary([BatchPayload0,
                                                    pulserl_io:write_int32(SerializedSingleMsgMetaSize),
                                                    SerializedSingleMsgMeta,
                                                    Payload]),
...
increment_sent_metric(#state{metrics = Metrics} = State, Increment) ->
    State#state{metrics =
                    Metrics#metrics{sent_messages = Metrics#metrics.sent_messages + Increment}}.

send_reply_to_all_waiters(Reply, State) ->
    SentRequestWaiters = [Waiters || {_, Waiters} <- maps:to_list(State#state.seqId2waiters)],
    PendingRequestWaiters =
        [Waiter || {Waiter, _} <- queue:to_list(State#state.batching_requests)],
    send_replies_to_waiters(Reply,
                            lists:flatten(SentRequestWaiters) ++ PendingRequestWaiters,
                            State).
...
send_replies_to_waiters(Reply, Waiters, State) ->
    lists:foreach(fun(Client) ->
                     try
                         ...
                         _:Reason ->
                             error_logger:error_msg("Error(~p) on replying to the client.",
                                                    [{Reason}])
                     end
                  end,
                  Waiters),
...
add_request_to_batch(Request,
                     #state{batching_requests = BatchingRequests,
                            max_pending_requests = MaxPendingMessages,
                            max_pending_requests_across_partitions =
                                MaxPendingPartitionedMessages} =
                         State) ->
    ...
    case RequestQueueLen < MaxPendingMessages
         andalso MaxPendingPartitionedMessages
                 > %% Increment by zero to read current count
                   update_pending_count_across_partitions(State, 0)
    of
        true ->
            %% Add to the `pending_requests` queue
            {CrossPartitionsPendingLen2, BatchRequestsLen2, NewBatchRequests} =
                add_request_to_batch2(Request, State),
            %% Check again if it's still not full
            if BatchRequestsLen2 < MaxPendingMessages
               andalso MaxPendingPartitionedMessages > CrossPartitionsPendingLen2 ->
...
add_request_to_batch2(Request, #state{batching_requests = RequestQueue} = State) ->
    ...
    {update_pending_count_across_partitions(State, 1),
     queue:len(NewRequestQueue),
     NewRequestQueue}.

update_pending_count_across_partitions(#state{topic = Topic}, Increment) ->
...
maybe_child_producer_exited(ExitedPid, Reason, State) ->
    case dict:find(ExitedPid, State#state.child_to_partition) of
        {ok, Partition} ->
            error_logger:warning_msg("Producer(~p) to '~s' exited abnormally "
                                     "due to reason. '~p'. Restarting...",
                                     [ExitedPid,
                                      topic_utils:partition_str(State#state.topic, Partition),
                                      Reason]),
            State2 =
                State#state{partition_to_child =
                                dict:erase(Partition, State#state.partition_to_child),
                            child_to_partition =
                                dict:erase(ExitedPid, State#state.child_to_partition)},
            case create_inner_producer(Partition, State2) of
                {_NewPid, #state{} = NewState} ->
                    error_logger:info_msg("Producer to '~s' restarted.",
                                          [topic_utils:partition_str(State#state.topic,
                                                                     Partition)]),
                    NewState;
...
initialize_children(#state{partition_count = NumberOfPartitions} = State) ->
    {State2, Err} =
        lists:foldl(fun(PartitionIndex, {State0, Error}) ->
                       if Error == ?UNDEF ->
                    ...
                    {State, ?UNDEF},
                    lists:seq(0, NumberOfPartitions - 1)),
    case Err of
        {error, _} = Err ->
            [pulserl_producer:close(Pid, false)
             || {_, Pid} <- dict:fetch_keys(State2#state.child_to_partition)],
            Err;
        ?UNDEF ->
            State2
    end.
...
establish_producer(#state{topic = Topic} = State) ->
    Command =
        #'CommandProducer'{topic = topic_utils:to_string(Topic),
                           producer_id = State#state.producer_id,
                           producer_name = State#state.producer_name,
                           metadata =
                               commands:to_con_prod_metadata(State#state.producer_properties)},
...
start_send_batch_timer(#state{batch_max_delay_ms = BatchDelay} = State) ->
    State#state{batch_send_timer = erlang:send_after(BatchDelay, self(), ?INFO_SEND_BATCH)}.
...
create_inner_producer(Retries, Index, #state{topic = Topic, options = Opts} = State) ->
    ...
    case pulserl_producer:start_link(PartitionedTopic, Opts) of
        {ok, Pid} ->
            {Pid,
             State#state{partition_to_child =
                             dict:store(Index, Pid, State#state.partition_to_child),
                         child_to_partition =
                             dict:store(Pid, Index, State#state.child_to_partition)}};
        {error, _} = Error ->
            case Retries > 0 of
                true ->
                    create_inner_producer(Retries - 1, Index, State);
                _ ->
                    Error
            end
    end.
...
notify_instance_provider_of_state(#state{topic = Topic} = State, Event) ->
