...
produce(PidOrTopic, Payload) when is_list(Payload) or is_binary(Payload) ->
...
produce(PidOrTopic, Payload, Options, Callback)
    when is_list(Payload) or is_binary(Payload)
         ...
         andalso (is_function(Callback) orelse Callback == ?UNDEF) ->
    produce(PidOrTopic, pulserl_producer:new_message(Payload, Options), Callback).

%%--------------------------------------------------------------------
%% @doc publish a message synchronously
%%--------------------------------------------------------------------
sync_produce(PidOrTopic, Payload) when is_list(Payload) orelse is_binary(Payload) ->
    sync_produce(PidOrTopic, pulserl_producer:new_message(Payload, []), ?UNDEF);
%%--------------------------------------------------------------------
%% @doc publish a message synchronously
%%--------------------------------------------------------------------
...
sync_produce(PidOrTopic, Payload, Options)
    when is_list(Payload) or is_binary(Payload) andalso is_list(Options) ->
    sync_produce(PidOrTopic, pulserl_producer:new_message(Payload, Options));
%%-------------------------------------------------------------------------------
