...
%% API
-export([is_partitioned/1, is_persistent/1, new_partition/2, parse/1, partition_index/1,
         partition_of/2, partition_str/2, to_string/1]).
        ...
        case binary:match(Topic, [<<"://">>]) of
            nomatch ->
                case binary:split(Topic, <<"/">>, [global]) of
                    ...
                    [_Name] ->
                        iolist_to_binary([?PERSISTENT_DOMAIN,
                                          "://",
                                          ?PUBLIC_TENANT,
                                          "/",
                                          ?DEFAULT_NAMESPACE,
                                          ...
                                          Topic]);
                    _ ->
                        error({bad_topic_name,
                               "Name must be in the format <topic>, "
                               "or [<tenant>/]<namespace>/<topic>"},
                              [Topic])
                end;
            _ ->
                Topic
...
new_partition(#topic{} = Parent, Index) when is_integer(Index) andalso Index >= 0 ->
