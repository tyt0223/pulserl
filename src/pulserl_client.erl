...
init([ServiceUrl, Config]) ->
    ...
    case parse_and_resolve_service_url(ServiceUrl, TlsEnable) of
        {error, Reason} ->
            {stop, Reason};
        Addresses ->
            {ServiceUrls, PhysicalAddress2CnxMap, Physical2LogicalAddressMap} =
                lists:foldl(fun({Hostname, IpAddress, Port},
                                {Urls, PhysicalAddress2CnxMap0, Physical2LogicalAddressMap0}) ->
                               LogicalAddress =
                                   pulserl_utils:to_logical_address(Hostname, Port, TlsEnable),
                               {sets:add_element(LogicalAddress, Urls),
                                maps:put({IpAddress, Port}, [], PhysicalAddress2CnxMap0),
                                maps:put({IpAddress, Port},
                                         LogicalAddress,
                                         Physical2LogicalAddressMap0)}
                            end,
                            {sets:new(), #{}, #{}},
                            Addresses),
            ets:new(partition_pending_messages,
                    [named_table,
                     set,
                     public,
...
get_partitioned_topic_meta(Topic, #state{meta_cnx = MetaCnxPid} = State) ->
    ...
    case pulserl_conn:send_simple_command(MetaCnxPid, Command) of
        #'CommandPartitionedTopicMetadataResponse'{response = 'Failed',
                                                   error = Error,
                                                   message = Msg} ->
            {{error, {Error, Msg}}, State};
        #'CommandPartitionedTopicMetadataResponse'{partitions = Partitions} ->
...
discover_address(Topic, Command, CnxPid, State) ->
    case pulserl_conn:send_simple_command(CnxPid, Command) of
        ...
        #'CommandLookupTopicResponse'{response = 'Failed',
                                      error = Error,
                                      message = Msg} ->
            {{error, {Error, Msg}}, State};
        {error, Reason} ->
...
get_connection_to_one_of_these_logical_addresses([LogicalAddress | Rest], State) ->
    case pulserl_utils:logical_to_physical_addresses(LogicalAddress, State#state.tls_enable)
    of
        {error, _} = Error ->
            Error;
        PhysicalAddresses ->
...
get_connection([PhysicalAddr | Rest],
               LogicalAddr,
    ...
    case {MaxConnectionsPerBroker, Connections} of
        ...
        {Max, Connections} when Max == length(Connections) ->
            ...
            {CurrConnection#cached_conn.pid,
             LogicalAddr,
...
parse_and_resolve_service_url(ServiceUrl, TlsEnable) ->
    ...
    of
        true ->
            {error, "Invalid service url: " ++ ServiceUrl2};
        _ ->
            Ls
    end.
        ...
        {ok, CnxPid} ->
            ...
            receive
                {?CONNECTION_UP_EVENT, SockAddr, _Socket, CnxPid} ->
                    ...
                    State1 =
                        update_physical_address_2_logical_address_map(SockAddr,
                                                                      LogicalAddress,
                                                                      State),
    ...
    case maps:take(CnxPid, State#state.connection_2_physical_address) of
        {PhysicalAddress, NewMap} ->
            ...
            update_physical_address_2_connections_map(PhysicalAddress,
                                                      #cached_conn{pid = CnxPid, active = false},
                                                      NewState);
        _ ->
            State
    end.
...
add_broker_connection(CnxPid, PhysicalAddress, State) ->
    ...
    NewState = State#state{connection_2_physical_address = NewMap},
    update_physical_address_2_connections_map(PhysicalAddress,
                                              #cached_conn{pid = CnxPid, active = true},
                                              NewState).
...
update_physical_address_2_logical_address_map(PhysicalAddress, LogicalAddress, State) ->
    ...
    State#state{physical_address_2_logical_address =
                    maps:put(PhysicalAddress, LogicalAddress, Map)}.

update_physical_address_2_connections_map(PhysicalAddress,
                                          #cached_conn{} = Conn,
                                          State) ->
    PhysicalAddress2Connections =
        maps:update_with(PhysicalAddress,
                         fun(Connections) ->
                            ...
                            case lists:keyfind(CnxPid, PidPos, Connections) of
                                false ->
                                    [Conn | Connections];
                                _ ->
                                    lists:keyreplace(CnxPid, PidPos, Connections, Conn)
                            end
                         end,
                         [Conn],
