...
%% API
-export([start_link/1]).
-export([close/1, create/1]).
-export([register_handler/3, send_payload_command/4, send_simple_command/2]).
...
register_handler(Pid, Handler, Type)
    when is_pid(Handler), Type == producer; Type == consumer ->
...
%% Call callback
handle_call({register_handler, Consumer, consumer},
            _From,
...
handle_call({register_handler, Producer, producer},
            _From,
...
handle_call({send_command, PulsarCommand}, From, State) ->
    case send_internal(PulsarCommand, State) of
        ...
        {noreply, WaiterId, NewState} when WaiterId /= ?UNDEF ->
            ...
            WaiterMonitor2Id =
                dict:store(WaiterMonitorRef, WaiterId, NewState#state.waiter_monitor2Id),
...
%% Cast callback
handle_cast({send_command, {payload, Command, _Metadata, _Payload} = PulsarCommand},
            State) ->
    case send_internal(PulsarCommand, State) of
...
parse_buffer_data(State) ->
    ...
    case pulserl_io:read_frame(Buffer) of
        ...
        {Frame, NewBuffer} ->
            ...
            State4 =
                lists:foldl(fun(Command, State3) ->
    ...
    case Command of
        ...
        _ ->
            error_logger:error_msg("No handler is implemeted to handle the "
                                   "command: ~p with header&payload: ~p",
                                   [Command, HeadersAndPayload]),
            State
    end.
...
handle_error(#'CommandError'{request_id = RequestId,
                             error = Error,
                             message = Msg},
             State) ->
    send_reply_to_request_waiter(RequestId, {error, {Error, Msg}}, State).

handle_send_receipt(#'CommandSendReceipt'{producer_id = ProdId,
                                          sequence_id = SequenceId,
                                          message_id = MessageId},
                    State) ->
    fetch_producer_by_id(ProdId,
                         State,
                         fun (Pid) when is_pid(Pid) ->
                                 safe_send(Pid, {ack_received, SequenceId, MessageId});
                             (_) ->
                                 error_logger:warning_msg("The producer with id: ~p not found when "
                                                          "send receipt is received",
                                                          [ProdId])
                         end).

handle_send_error(#'CommandSendError'{producer_id = ProdId,
                                      sequence_id = SequenceId,
                                      error = Error,
                                      message = Message},
                  State) ->
...
handle_producer_success(#'CommandProducerSuccess'{request_id = RequestId} = Response,
                        State) ->
...
handle_close_consumer(#'CommandCloseConsumer'{consumer_id = ConsumerId}, State) ->
    fetch_consumer_by_id(ConsumerId,
                         State,
                         fun (Pid) when is_pid(Pid) ->
                                 pulserl_consumer:close(Pid, true);
                             (_) ->
                                 error_logger:warning_msg("The consumer with id: ~p not found while "
                                                          "closing",
                                                          [ConsumerId])
                         end).
...
handle_close_producer(#'CommandCloseProducer'{producer_id = ProdId}, State) ->
    fetch_producer_by_id(ProdId,
                         ...
                         fun (Pid) when is_pid(Pid) ->
                             ...
                             (_) ->
                                 error_logger:warning_msg("The producer with id: ~p not found while "
                                                          "closing",
                                                          [ProdId])
                         end).

handle_lookup_topic(#'CommandLookupTopicResponse'{request_id = RequestId} = Response,
                    State) ->
    send_reply_to_request_waiter(RequestId, Response, State).

handle_partition_topic_metadata(#'CommandPartitionedTopicMetadataResponse'{request_id =
                                                                               RequestId} =
                                    Response,
                                State) ->
    send_reply_to_request_waiter(RequestId, Response, State).

handle_error_prod_cons_send(#'CommandSend'{producer_id = ProdId,
                                           sequence_id = SequenceId},
                            {error, Reason},
                            State) ->
    handle_send_error(#'CommandSendError'{producer_id = ProdId,
...
handle_message(#'CommandMessage'{consumer_id = ConsumerId,
                                 message_id = MsgId,
                         ...
                         fun (Pid) when is_pid(Pid) ->
                                 safe_send(Pid,
                                           {new_message,
                                            ...
                                            HeadersAndPayload});
                             (_) ->
                                 error_logger:warning_msg("The consumer with id: ~p not found when "
                                                          "message arrive",
                                                          [ConsumerId])
                         end).
...
send_reply_to_all_request_waiters(Reply, State) ->
    lists:foldl(fun(WaiterId, State0) -> send_reply_to_request_waiter(WaiterId, Reply, State0)
                end,
                State,
                dict:fetch_keys(State#state.waiters)).
...
send_reply_to_request_waiter(WaiterId, Reply, State) ->
    case dict:take(WaiterId, State#state.waiters) of
        ...
        _ ->
            error_logger:warning_msg("A reply was sent but it's not associated "
                                     "with any waiter"),
            State
    end.
...
send_pong_to_broker(#state{socket = Sock} = State) ->
    case do_send_internal(#'CommandPong'{}, false, State) of
        {_, {error, _} = Error, NewState} ->
            error_logger:info_msg("Pong reply failed for connection(~p, "
                                  "~p).Error: ~p",
                                  [Sock, self(), Error]),
            NewState;
        {_, _, NewState} ->
...
do_send_internal2(RequestData,
                  RequestId,
                  #state{socket = Sock, socket_module = SockMod} = State) ->
...
schedule_reconnect(#state{connect_attempts = Attempts} = State) ->
    ...
    error_logger:info_msg("The next reconnection attempt(~p) will "
                          "be in ~p milliseconds",
                          [Attempts + 1, NextReconnectDelay]),
...
create_connection(#state{logical_address = LogicalAddress, socket_module = SockMod} =
                      State) ->
...
connect_to_broker(#state{socket_module = SockMod, connect_timeout = ConnectTimeout} =
                      State,
                  IpAddress,
                  Port) ->
    TcpOptions = [binary, {active, false} | State#state.tcp_options],
    error_logger:info_msg("Connecting to broker ~s:~p",
                          [erlwater_net:ip_to_binary(IpAddress), Port]),
    case SockMod:connect(IpAddress, Port, TcpOptions, ConnectTimeout) of
        {ok, Socket} ->
            {ok,
             State#state{connect_attempts = 0,
                         socket_address = {IpAddress, Port},
                         socket = optimize_socket(State, Socket)}};
        {error, Reason} = Error ->
            error_logger:error_msg("Unable to connect to broker at: ~s. "
                                   "Reason: ~p",
                                   [pulserl_utils:sock_address_to_string(IpAddress, Port), Reason]),
            Error
    end.
        ...
        {_, _, NewState} ->
            case SockMod:recv(Socket, 0) of
                {ok, Data} ->
                    ...
                    case Command of
                        #'CommandConnected'{server_version = ServerVsn,
                            ...
                            {ok,
                             NewState#state{state = ?STATE_READY,
                                            ...
                                            protocol_version = ProcVsn,
                                            max_message_size = MaxMessageSize}};
                        #'CommandError'{error = Error, message = ErrorMessage} ->
                            {error, {Error, ErrorMessage}}
                    end;
                {error, Reason} ->
                    error_logger:error_msg("Error making the connect handsake. Reason: ~p",
                                           [Reason]),
                    {error, Reason}
            end
    end.
...
optimize_socket(#state{tcp_options = TcpOpts} = State, Sock) ->
    ...
    case {proplists:get_value(sndbuf, TcpOpts, ?UNDEF),
          proplists:get_value(recbuf, TcpOpts, ?UNDEF)}
    of
        {?UNDEF, ?UNDEF} ->
            SockOptsMod = socket_option_module(State),
...
notify_client_of_down(Error,
                      #state{socket_address = SockAddr, socket = Socket} = State) ->
